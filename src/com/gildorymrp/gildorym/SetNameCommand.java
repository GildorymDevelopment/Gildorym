...
public class SetNameCommand implements CommandExecutor {
	...
	public SetNameCommand(Gildorym gildorym, MySQLDatabase sqlDB) {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		String name = essentials.getUser(sender.getName()).getNickname();
		...
		if (args.length <= 5) {
			if (args.length == 0) {
				if (sender.hasPermission("gildorym.setname") && sender instanceof Player) {
					...
					name = essentials.getUser(sender.getName()).getNickname();
